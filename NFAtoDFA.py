...
nfa = {}                                                #Taking NFA input from User    

file1 = open('NFAtoDFA.txt', 'r')                         #reading the text file
Lines = file1.readlines() 

transitions = str(Lines[1]).split()                     #no of transitions
tlen = len(transitions)

states = str(Lines[2]).split()
slen = len(states)

n = slen                                                #Enter total no. of states
t = tlen                                                #Enter total no. of transitions/paths eg: a,b so input 2 for a,b,c input 3

table = Lines[4:-2]
rem = len(Lines) - 6                                    #Total no. of routes

for i in range(0,rem,t):  
    state = table[i][0]                                 #Enter state name eg: A, B, C, q1, q2 ..etc
    nfa[state] = {}                                     #Creating a nested dictionary
    for j in range(t):
        path = table[j][2]                              #Enter path eg : a or b in {a,b} 0 or 1 in {0,1}
        reaching_state = table[i+j][4:].split()         #Enter all the end states that 
...
nfa_final_state =  Lines[-1]                            # Enter final state/states of NFA
