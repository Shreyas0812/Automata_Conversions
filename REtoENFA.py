...
def printStateTransitions(state, states_done, symbol_table, fobj):
    ...
    for symbol in list(state.next_state):
        ...
        print(line_output)
        fobj.write(line_output+'\n')
        #Third ip/ final table

        for ns in state.next_state[symbol]:
            printStateTransitions(ns, states_done, symbol_table, fobj=fobj)
        
def printTransitionTable(finite_automata, fobj):
    print("State\t\tSymbol\t\t\tNext state")
    printStateTransitions(finite_automata[0], [], {finite_automata[0]:0}, fobj = fobj)
...
for i in r:
    if i>='a' and i<='z':
        if i not in unique_letter:
            ...
            letters.append(i)
print(unique_letter, len(unique_letter))    #first ip
letters.append('e')
print(letters)  #second ip

start_state = '1' #4th input
num_final_state = '1' #Number of Final States

fobj = open("REtoENFA.txt","w")
fobj.write(str(len(unique_letter)+1)+'\n')
for i in letters:
    fobj.write(i+'\n')

printTransitionTable(fa, fobj)
